    ...
    public class UsuarioRolesDAC : DataAccessComponent, IRepository<UsuarioRoles>
    {
        private UsuarioRoles LoadUsuarioRoles(IDataReader dr)
        {
            try
            {
                ...
                usuarioRoles.roles.tipo = GetDataValue<string>(dr, "tipo");
                return usuarioRoles;
            }
            catch (Exception e)
            {
        ...
        private UsuarioRoles LoadUsuarioRolesSimplificado(IDataReader dr)
        {
            try
            {
                UsuarioRoles usuarioRoles = new UsuarioRoles();
        ...
        public void Delete(UsuarioRoles entity)
        {
            ...
            {
                db.AddInParameter(cmd, "@UserId", DbType.Int32, entity.usuarios.Id);
                db.AddInParameter(cmd, "@RoleId", DbType.Int64, entity.roles.Id);
                db.ExecuteNonQuery(cmd);
            }
        }
        public void Delete(int id)
        {
            const string SQL_STATEMENT = "delete from AspNetUserRoles where UserId=@UserId";
            var db = DatabaseFactory.CreateDatabase(CONNECTION_NAME);
            using (DbCommand cmd = db.GetSqlStringCommand(SQL_STATEMENT))
            {
        ...
        public List<UsuarioRoles> ReadbyPregunta(int usuario)
        {
            const string SQL_STATEMENT = "select * from AspNetRoles  as anr join AspNetUserRoles as aur on aur.RoleId =anr.Id where activo=1 and tipo='pregunta' and UserId=@Id";

            List<UsuarioRoles> result = new List<UsuarioRoles>();
            var db = DatabaseFactory.CreateDatabase(CONNECTION_NAME);
            using (DbCommand cmd = db.GetSqlStringCommand(SQL_STATEMENT))
            {
                db.AddInParameter(cmd, "@Id", DbType.Int32, usuario);
                using (IDataReader dr = db.ExecuteReader(cmd))
                {
                    while (dr.Read())
                    {
                        UsuarioRoles roles = LoadUsuarioRolesSimplificado(dr);
                        result.Add(roles);
                    }
                }
            }
